...
os.environ['SDL_VIDEO_X11_WMCLASS'] = 'hardwarequest'
os.environ['SDL_VIDEO_WAYLAND_WMCLASS'] = 'hardwarequest'
...
def registrar_desktop_linux():
    ...
    if sys.platform.startswith("linux"):
        ...
        src_icon = obter_caminho_recurso(os.path.join("assets", "img", "logo_iot.png"))
        dest_icon = os.path.join(icon_dir, "hardwarequest.png")
        if os.path.exists(src_icon):
            try:
                shutil.copyfile(src_icon, dest_icon)
            except Exception:
                pass
        dest_file = os.path.join(dest_dir, "hardwarequest.desktop")
        exec_path = os.path.abspath(sys.argv[0])
        if exec_path.endswith(".py"):
            exec_line = f"{sys.executable} {exec_path}"
        else:
            exec_line = exec_path
...
def desenhar_trofeu(superficie, x, y, tamanho=24):
    cor_ouro = (255, 215, 0)
    cor_ouro_escuro = (218, 165, 32)
    pygame.draw.polygon(superficie, cor_ouro, [
        (x + tamanho // 4, y + tamanho // 4),
        (x + 3 * tamanho // 4, y + tamanho // 4),
        (x + 5 * tamanho // 8, y + 5 * tamanho // 8),
        (x + 3 * tamanho // 8, y + 5 * tamanho // 8)
    ])
    pygame.draw.rect(superficie, cor_ouro, (x + 7 * tamanho // 16, y + 5 * tamanho // 8, tamanho // 8, 2 * tamanho // 8))
    pygame.draw.rect(superficie, cor_ouro_escuro, (x + tamanho // 4, y + 7 * tamanho // 8, tamanho // 2, tamanho // 8))
    pygame.draw.lines(superficie, cor_ouro, False, [
        (x + 3 * tamanho // 8, y + 3 * tamanho // 8),
        (x + tamanho // 8, y + 3 * tamanho // 8),
        (x + tamanho // 8, y + tamanho // 2),
        (x + 3 * tamanho // 8, y + 5 * tamanho // 8)
    ], 2)
    pygame.draw.lines(superficie, cor_ouro, False, [
        (x + 5 * tamanho // 8, y + 3 * tamanho // 8),
        (x + 7 * tamanho // 8, y + 3 * tamanho // 8),
        (x + 7 * tamanho // 8, y + tamanho // 2),
        (x + 5 * tamanho // 8, y + 5 * tamanho // 8)
    ], 2)
...
# ==========================================
registrar_desktop_linux()
pygame.init()
LARGURA, ALTURA = 950, 680
icon_surface = pygame.image.load(obter_caminho_recurso(os.path.join("assets", "img", "logo_iot.png")))
...
class PlacarGlobal:
    ...
    def adicionar_pontuacao(self, nome, pontos):
        ...
        for item in self.pontuacoes:
            if item["nome"].lower() == nome.strip().lower():
                item["nome"] = nome.strip()
                item["pontos"] = int(pontos)
                encontrado = True
                break
        if not encontrado:
            self.pontuacoes.append({"nome": nome.strip(), "pontos": int(pontos)})
    ...
    def desenhar(self, superficie, x, y, largura, altura):
        ...
        txt_x = x + (largura - titulo.get_width()) // 2
        txt_y = y + 15
        superficie.blit(titulo, (txt_x, txt_y))
        desenhar_trofeu(superficie, txt_x - 35, txt_y, 28)
        desenhar_trofeu(superficie, txt_x + titulo.get_width() + 10, txt_y, 28)
